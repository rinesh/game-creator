...
`$kaplay` uses the project already open in the WebMCP-enabled Kaplayground page. It validates the page-owned Contract 1.1 guide, negotiates the tools and revision-guard schemas that are actually available, and fetches focused live references only when the task needs them. Full deployments support requested example changes, project assets, exact Asset Brew loader code, conflict-safe file edits, preview diagnostics, run-scoped evidence, screenshots, gameplay input, and intent-preserving persistence; reduced adapters can still perform the supported subset instead of failing an exact tool-count check. If revision-safe editing exists without preview verification, the skill asks before making a source-only edit and makes no runtime claim.
