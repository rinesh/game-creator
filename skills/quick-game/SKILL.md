...
If the arguments explicitly name KAPLAY, Kaplayground, Kaplayground WebMCP, or `rinesh/kaplayground`, stop and direct the user to the separate [Kaplayground plugin](https://github.com/rinesh/kaplayground/tree/kaplayground-plugin-v1.5.0/plugins/kaplayground). Do not claim a local `kaplay` skill, copy a Phaser/Three.js template, or start another dev server.
